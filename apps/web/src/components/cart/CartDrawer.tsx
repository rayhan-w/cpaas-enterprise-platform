  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end animate-fade-in">
      ...
      <div className="relative w-full max-w-md bg-[#F8F7F5] h-full shadow-2xl flex flex-col z-10 animate-slide-in-right">
        ...
        {/* Free Shipping Meter */}
        <div className="bg-[#F1F8E8] border-b border-[#EDE5E1] p-3 text-xs">
          <div className="flex items-center justify-between mb-1.5 font-medium text-[#1A1512]">
            <span className="flex items-center gap-1 text-[11px]">
              <Truck className="w-3.5 h-3.5 text-[#6CAE14]" />
              {remainingForFree === 0 ? (
                <span className="text-[#7A9C78] font-bold">You qualify for FREE Delivery! 🎉</span>
              ) : (
                <span>
                  Add <strong className="text-[#6CAE14]">{formatPrice(remainingForFree)}</strong> more for FREE delivery
                </span>
              )}
            </span>
            <span className="font-bold text-[10px] text-[#6CAE14]">{progressPercent}%</span>
          </div>
          <div className="w-full h-1.5 bg-[#EDE5E1] rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#6CAE14] to-[#7A9C78] transition-all duration-300"
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 text-[#9B8A86]">
              ...
              <button
                onClick={closeCart}
                className="mt-4 bg-[#6CAE14] hover:bg-[#5B960E] text-white text-xs font-semibold px-5 py-2.5 rounded-full transition-colors"
              >
                Start Shopping
              </button>
            </div>
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      <Link
                        ...
                        onClick={closeCart}
                        className="text-xs font-semibold text-[#1A1512] hover:text-[#6CAE14] transition-colors line-clamp-1"
                      >
                        {item.product.name}
                      </Link>
                      <button
          ...
          <div className="p-4 bg-white border-t border-[#EDE5E1] space-y-3 shadow-lg">
            ...
            {appliedCoupon ? (
              ...
              <form onSubmit={handleApplyCoupon} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#F8F7F5] border border-[#EDE5E1] rounded-xl px-3 py-2 text-xs text-[#1A1512] uppercase placeholder-[#9B8A86] focus:outline-none focus:border-[#6CAE14]"
                />
                <button
                  type="submit"
            ...
            <div className="space-y-1.5 text-xs text-[#6B5B58] pt-1">
              ...
              <div className="flex justify-between text-sm font-bold text-[#1A1512] pt-2 border-t border-[#F2EDEA]">
                ...
                <span className="text-base text-[#6CAE14]">{formatPrice(total)}</span>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-1">
              <Link
                href="/checkout"
                onClick={closeCart}
                className="w-full flex items-center justify-center gap-2 bg-[#6CAE14] hover:bg-[#5B960E] text-white py-3 px-4 rounded-xl text-xs font-bold transition-all shadow-md active:scale-95 text-center"
              >
