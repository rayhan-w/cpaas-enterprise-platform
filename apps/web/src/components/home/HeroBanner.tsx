    ...
    <div className="relative max-w-7xl mx-auto px-4 pt-4 pb-2">
      <div className="relative h-[380px] sm:h-[440px] lg:h-[480px] rounded-3xl overflow-hidden shadow-elevation-3 border border-[#EDE5E1] bg-[#1A1512]">
        ...
        <div className="relative z-10 h-full flex flex-col justify-center max-w-xl p-6 sm:p-12 text-white">
          {current.badge && (
            <div className="inline-flex items-center gap-1.5 bg-[#C4737E]/90 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-4 w-fit shadow-sm">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <Link
              ...
              className="inline-flex items-center gap-2 bg-[#C4737E] hover:bg-[#A85862] text-white font-semibold text-sm px-6 py-3 rounded-full transition-all shadow-lg group active:scale-95"
            >
              <span>{current.ctaText}</span>
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </Link>

            <div className="hidden sm:flex items-center gap-2 text-xs text-white/70 bg-white/10 backdrop-blur-xs px-3 py-2 rounded-full border border-white/15">
              <ShieldCheck className="w-4 h-4 text-[#7A9C78]" />
          ...
          {banners.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                i === currentIdx ? 'w-8 bg-[#C4737E]' : 'w-2 bg-white/40 hover:bg-white/70'
              }`}
